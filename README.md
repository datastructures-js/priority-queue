...
### fromArray
If the queue is being created from an existing array, and there is no desire to use an extra O(n) space, this static function can turn an array into a priority queue. If the existing array should not be mutated, simply create the queue from a copy of it or add array values to an empty queue using enqueue.
