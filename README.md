...
### fromArray
If the queue is being created from an existing array, and there is no desire to use an extra space, this static function can be used to turn an array into a priority queue. If the existing array should not be mutated, simply create the queue from a copy of it.
